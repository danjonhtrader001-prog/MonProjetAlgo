## Project: quant-finance-probability
...
- **Fibonacci retracement & extension** (`4_quantitative_finance/fibonacci_retracement.py`): Fibonacci levels on simulated price swings, GBM paths, and Monte Carlo level‑touch test.
...
The **Fibonacci sequence** \( 0, 1, 1, 2, 3, 5, 8, 13, \ldots \) has the property that the ratio of consecutive terms converges to the **golden ratio** \( \varphi \approx 1.618 \). From this ratio, traders derive a set of **retracement levels** (23.6 %, 38.2 %, 50 %, 61.8 %, 78.6 %) and **extension levels** (127.2 %, 161.8 %, 261.8 %) that subdivide a prior price swing.

#### Link with algorithmic trading

After an impulse move (upward or downward), these levels are used as **potential support/resistance zones** and **profit targets**. Because many market participants watch the same levels, order flow tends to cluster around them, making Fibonacci levels a **self-reinforcing** technical analysis tool. In quantitative systems, they can serve as inputs for signal generation, stop-loss placement, and risk management.

#### Python implementation

The script `4_quantitative_finance/fibonacci_retracement.py`:

1. Demonstrates the Fibonacci sequence and convergence to \( \varphi \).
2. Simulates a GBM price path with a clear uptrend followed by a retracement.
3. Automatically detects swing high / swing low and computes all Fibonacci levels.
4. Plots the price path with labeled retracement & extension lines.
5. Runs a **Monte Carlo experiment** (5 000 paths) to measure how often the retracement low coincides with each Fibonacci level under a purely random walk, highlighting the difference between mathematical coincidence and market microstructure effects.
...
python 4_quantitative_finance/fibonacci_retracement.py
